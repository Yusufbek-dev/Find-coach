<template>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{invalid:!firstName.isValid}">
      ...
      <p v-if="!firstName.isValid" :style="{'color':'red'}">FirstName not must be empty.</p>
    </div>
    <div class="form-control" :class="{invalid:!lastName.isValid}">
      <label for="LastName">LastName</label>
      <input type="text" id="firstName" v-model.trim="lastName.val" @blur="clearValidity('lastName')" />
      <p v-if="!lastName.isValid" :style="{'color':'red'}">LastName not must be empty.</p>
    </div>
    <div class="form-control" :class="{invalid:!description.isValid}">
      <label type="text" id="description">Description</label>
      <textarea id="description" rows="5" v-model.trim="description.val" @blur="clearValidity('description')"></textarea>
      <p v-if="!description.isValid" :style="{'color':'red'}">Description not must be empty.</p>
    </div>
    ...
    <div class="form-control" :class="{invalid:!areas.isValid}">
      ...
      <p v-if="!areas.isValid" :style="{'color':'red'}">At least one expertise must be selected.</p>
    </div>
    <p v-if="!formIsValid" :style="{'color':'red'}">Please fix the above errors and submit again.</p>
